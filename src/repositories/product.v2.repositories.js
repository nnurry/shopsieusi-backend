...
// note for future development: consider PREPARING statements to optimize query plans
class ProductRepository {
    static async findProductById(productId) {
        try {
            const res = await db.sql`
                ...
                LEFT JOIN public.product_variants v ON p.product_id = v.product_id
                LEFT JOIN public.variant_attributes va ON va.attribute_id = v.variant_id
                LEFT JOIN public.variant_attribute_values vav ON v.variant_id = vav.variant_id
                LEFT JOIN public.product_variant_prices vpr ON v.variant_id = vpr.variant_id
                LEFT JOIN public.product_variant_stocks vs ON v.variant_id = vs.variant_id

                WHERE p.product_id = ${productId} AND p.is_deleted = false
                GROUP BY
                    p.product_id, p.name, p.slug, 
                    p.created_at, p.updated_at, p.is_deleted, v.variant_id, c.category_id
                ;
            `;
            if (res.length !== 0) {
                    ...
                    product: {
                        ...
                        variants: productData.variants.map(variant => {
                            ...
                            return {
                                ...
                                attributeValues: variant.attribute_value,
                                price: variant.price,
                                stock: variant.stock
                            };
                        }),
    ...
    static async createProduct({ name, attributeNames, images, categories, variants }) {
        try {
            const createResult = await db.sql.begin(async sql => {
                ...
                const imageIds = await sql.savepoint(sql => {
                    ...
                    INSERT INTO product_images ${sql(
                        payload,
                        "product_id", "image_key", "image_url", "image_type", "hosting_type",
                    )}
                    RETURNING id
                `}).then(res => res.map(val => val.id));


                const categoryIds = await sql.savepoint(sql => {
                    ...
                    return sql`
                    INSERT INTO products_categories ${sql(
                        payload,
                ...
                const attributeIds = await sql.savepoint(sql => {
                    const attributes = attributeNames.map(name => { return { attribute_name: name } });
                    console.log(attributes);
                    return sql`
                        INSERT INTO variant_attributes ${sql(attributes, "attribute_name")}
                        RETURNING attribute_id
                `}).then(res => res.map(val => val.attribute_id));

                const variantIds = await sql.savepoint(async (sql) => {
                    const variantIdPromises = variants.map(async (variant) => {
                        const [{ variant_id: variantId }] = await sql`
                            INSERT INTO product_variants (product_id)
                            VALUES (${productId})
                            RETURNING variant_id
                        `;
                        console.log("before payload", variantId);
                        const payloadPromise = await variant.attributes.map(async (attribute, idx) => {
                            console.log("attribute:", attribute, attributeIds);
                            // attribute = [1,2,3,4,5] -> array
                            const attributeId = attributeIds[idx];
                        ...
                        const payload = await Promise.all(payloadPromise);
                        console.log("after payload", payload);
                        let val = await sql`
                            INSERT INTO variant_attribute_values ${sql(payload)}
                            RETURNING value_id
                        `;
                        console.log("after variant_attribute_values", val);
                        val = await sql`
                            INSERT INTO product_variant_prices (variant_id, price)
                            VALUES (${variantId}, ${variant.price})
                            RETURNING price_id
                        `;
                        console.log("after product_variant_prices", val);
                        val = await sql`
                            INSERT INTO product_variant_stocks (variant_id, stock)
                            VALUES (${variantId}, ${variant.stock})
                            RETURNING stock_id
                        `;
                        console.log("after product_variant_stocks", val);
                        return variantId;
                    })
                    return Promise.all(variantIdPromises);
                }).then(res => res.map(val => val.variant_id));
